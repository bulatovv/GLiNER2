...
class _GoldContext(NamedTuple):
    """Everything needed to resolve one sample's gold values against its text."""
...
def _parse_gold(value: Any) -> Any:
    """Convert span mappings to :class:`SurfaceSpan`.

    Surfaces, ``None`` and ``""`` are returned unchanged.
    """
...
class SchemaTransformer:
    ...
    def _resolve_gold(self, value: Any, ctx: _GoldContext) -> List[Tuple[int, int]]:
        """Resolve one gold value to inclusive word-token spans.

        ``[(-1, -1)]`` marks a value that was annotated but could not be located.
        """
